...
async def _run_mesa_async(
    ...
    arquitecto = AssistantAgent(
        ...
        system_message=(
            ...
            f"1. Usa PRIMERO los datos que ya existen en Neo4j/Qdrant (ver esquema arriba).\n"
            f"2. SOLO llama a OpenAlex/Scopus/web para datos que NO estén en las bases.\n"
            f"3. Solo propón pasos que usen las herramientas listadas. NUNCA inventes otras.\n"
            f"4. Para Neo4j usa CONTAINS para nombres de personas:\n"
            f"   WHERE toLower(a.name) CONTAINS toLower('apellido')\n"
            f"5. Los tópicos en Neo4j están en inglés. Traduce siempre.\n"
            f"6. Para búsqueda semántica: search_scientific_papers_semantic con entity_context=\"{{ENTITY}}\".\n"
            f"7. get_author_coauthors_graph REQUIERE nombre de INVESTIGADOR (persona), NO de institución.\n"
            f"8. REGLA CRÍTICA: Python_CodeExecutor NO puede llamar a otras herramientas como \n"
            f"   query_knowledge_graph_cypher. Extrae primero con Cypher, luego pasa los\n"
            f"   resultados directamente al bloque Python como un string JSON o tabla Markdown.\n"
            f"   NUNCA escribas código Python que lea archivos CSV que no se hayan generado antes.\n"
            f"9. DATOS QUE NO EXISTEN en el grafo (NO los pidas, diseña sin ellos):\n"
            f"   - Relaciones SDG/ODS (no hay nodos :SDG ni :ODS)\n"
            f"   - Propiedades gender o ethnicity en los autores\n"
            f"   - Archivos CSV preexistentes (papers_citations.csv, ods_publications.csv, etc.)\n"
            f"   - Propiedades impact_factor o quartile en los papers\n"
            f"10. Si un objetivo no puede cumplirse con datos disponibles, dícelo explícitamente\n"
            f"    y diseña el paso con los datos reales que sí existen.\n"
            f"11. Cuando termines el script completo, escribe: SCRIPT_TÉCNICO_LISTO"
        ),
    )
