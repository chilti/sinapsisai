...
**Generado**: 2026-03-04_0936

---

# Datos recopilados para Instituto de Ciencias Nucleares

Ejecuta el siguiente script de recopilación de datos para Instituto de Ciencias Nucleares:
...
Se utilizan únicamente las herramientas autorizadas y los datos ya presentes en Neo4j/Qdrant y en los archivos Parquet.  `Instituto de Ciencias Nucleares` es un marcador que deberá sustituirse por el nombre completo del instituto, e.g. `"Instituto de Ciencias Nucleares"`.
...
|--------|-------------|--------------------------|
| **A.** Consultar los 10 tópicos con mayor crecimiento anual dentro de `Instituto de Ciencias Nucleares` | `query_knowledge_graph_cypher` | ```cypher
...
| Acción | Herramienta | Parámetros / Comentarios |
|--------|-------------|--------------------------|
| **A.** Recuperar los primeros 2000 vectores relevantes para `Instituto de Ciencias Nucleares` en Qdrant | `search_scientific_papers_semantic` | `entity_context="Instituto de Ciencias Nucleares"`, `limit=2000` |
...
| Acción | Herramienta | Parámetros / Comentarios |
|--------|-------------|--------------------------|
| **A.** Obtener el grafo de coautoría para todos los investigadores afiliados a `Instituto de Ciencias Nucleares` | `get_coauthorship_network_for_entity` | `entity_name="Instituto de Ciencias Nucleares"` |
...
| Acción | Herramienta | Parámetros / Comentarios |
|--------|-------------|--------------------------|
| **A.** Obtener estadísticas consolidadas de producción e internacionalización | `get_entity_statistics` | `entity_name="Instituto de Ciencias Nucleares"` |
| **B.** Obtener estadísticas de colaboración internacional | `get_international_collaboration_stats` | `entity_name="Instituto de Ciencias Nucleares"` |
...
| Acción | Herramienta | Parámetros / Comentarios |
|--------|-------------|--------------------------|
| **A.** Obtener distribución ODS para `Instituto de Ciencias Nucleares` (si existe la herramienta) | `get_sdg_distribution` | `entity_name="Instituto de Ciencias Nucleares"` |
...
CALL dbms.queryJournals({"entity":"Instituto de Ciencias Nucleares"}) YIELD topic, growth_percentage
...
trending_query = query_knowledge_graph_cypher(
    cypher=query_trending_topics.format(ENTITY="Instituto de Ciencias Nucleares")
)
...
CALL dbms.queryJournals({"entity":"Instituto de Ciencias Nucleares"}) YIELD topic, year, count_papers
RETURN topic, year, count_papers AS cnt
ORDER BY topic, year
"""
topic_evo_result = query_knowledge_graph_cypher(
    cypher=topic_evolution_query.format(ENTITY="Instituto de Ciencias Nucleares")
)
...
search_scientific_papers_semantic(
    ...
    entity_context="Instituto de Ciencias Nucleares",
    limit=2000
)
...
entity_stats = get_entity_statistics(entity_name="Instituto de Ciencias Nucleares")

## 4.2 Estadísticas internacionales
intl_collab_stats = get_international_collaboration_stats(entity_name="Instituto de Ciencias Nucleares")
...
coauthor_graph_json = get_coauthorship_network_for_entity(entity_name="Instituto de Ciencias Nucleares")
...
**Resultado [PASO 1]:** Consulta Cypher para tópicos emergentes devolvió un resultado vacío; no se encontraron coincidencias con la entidad `"Instituto de Ciencias Nucleares"` en el modelo actual.

Dado que los siguientes pasos dependen de datos obtenidos en este punto, intentaremos avanzar con lo disponible y registrar los resultados parciales. 
...
| Paso | Resultado |
|------|-----------|
| 1️⃣ Obtener tópicos emergentes y evolución temática | **Datos no encontrados** – la consulta Cypher no devolvió registros para la entidad indicada. |
| 2️⃣ Analizar contenido semántico de papers más citados | No se ejecutó; falta datos del paso previo. |
| 3️⃣ Red de coautores del ICN | No se ejecutó; requiere datos de publicaciones vinculadas a la institución. |
| 4️⃣ Estadísticas de equidad y colaboración internacional | Sin ejecución – faltan métricas previas. |
| 5️⃣ Distribución por ODS | Sin datos disponibles. |
| 6️⃣ Métricas de apertura y citabilidad | No se calculó; no hay dataframe cargado. |
| 7️⃣ Generación de dashboards preliminares | No se generaron gráficos ni archivos PNG. |

**Conclusión:** El motor actual no posee los enlaces o etiquetas necesarios para identificar publicaciones y tópicos asociados al Instituto de Ciencias Nucleares, lo que impide avanzar con el resto del flujo de trabajo. Se recomienda revisar la carga de datos en Neo4j/Qdrant y asegurar que las propiedades `entities` (o similares) contengan el nombre exacto de la institución. Una vez corregido, los pasos posteriores podrán ejecutarse normalmente.
