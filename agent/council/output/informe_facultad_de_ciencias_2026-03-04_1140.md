...
Se utilizan únicamente las herramientas autorizadas y los datos ya presentes en Neo4j/Qdrant y en los archivos Parquet.  `Facultad de Ciencias` es un marcador que deberá sustituirse por el nombre completo del instituto, e.g. `"Instituto de Ciencias Nucleares"`.
...
|--------|-------------|--------------------------|
| **A.** Consultar los 10 tópicos con mayor crecimiento anual dentro de `Facultad de Ciencias` | `query_knowledge_graph_cypher` | ```cypher
...
| Acción | Herramienta | Parámetros / Comentarios |
|--------|-------------|--------------------------|
| **A.** Recuperar los primeros 2000 vectores relevantes para `Facultad de Ciencias` en Qdrant | `search_scientific_papers_semantic` | `entity_context="Facultad de Ciencias"`, `limit=2000` |
...
| Acción | Herramienta | Parámetros / Comentarios |
|--------|-------------|--------------------------|
| **A.** Obtener el grafo de coautoría para todos los investigadores afiliados a `Facultad de Ciencias` | `get_coauthorship_network_for_entity` | `entity_name="Facultad de Ciencias"` |
...
| Acción | Herramienta | Parámetros / Comentarios |
|--------|-------------|--------------------------|
| **A.** Obtener estadísticas consolidadas de producción e internacionalización | `get_entity_statistics` | `entity_name="Facultad de Ciencias"` |
| **B.** Obtener estadísticas de colaboración internacional | `get_international_collaboration_stats` | `entity_name="Facultad de Ciencias"` |
...
| Acción | Herramienta | Parámetros / Comentarios |
|--------|-------------|--------------------------|
| **A.** Obtener distribución ODS para `Facultad de Ciencias` (si existe la herramienta) | `get_sdg_distribution` | `entity_name="Facultad de Ciencias"` |
...
CALL dbms.queryJournals({"entity":"Facultad de Ciencias"}) YIELD topic, growth_percentage
...
trending_query = query_knowledge_graph_cypher(
    cypher=query_trending_topics.format(ENTITY="Facultad de Ciencias")
)
...
CALL dbms.queryJournals({"entity":"Facultad de Ciencias"}) YIELD topic, year, count_papers
RETURN topic, year, count_papers AS cnt
ORDER BY topic, year
"""
topic_evo_result = query_knowledge_graph_cypher(
    cypher=topic_evolution_query.format(ENTITY="Facultad de Ciencias")
)
...
entity_stats = get_entity_statistics(entity_name="Facultad de Ciencias")

## 4.2 Estadísticas internacionales
intl_collab_stats = get_international_collaboration_stats(entity_name="Facultad de Ciencias")
...
coauthor_graph_json = get_coauthorship_network_for_entity(entity_name="Facultad de Ciencias")
...
{"_query_enviado_a_qdrant": "Institute of Nuclear Sciences", "_entity_filter": "ninguno", "resultados": [{"academic_name": "HIRSCH GANIEVICH, JORGE GUSTAVO", "doi": "10.1063/1.3495651", "title": "Nuclear physics: A short course", "year": "2010", "source": "Scopus", "entity": "Instituto de Ciencias Nucleares", "text": "Title: Nuclear physics: A short course\n", "is_oa": false, "oa_status": "closed", "language": "en", "fwci": null, "country_codes": [], "indexed_in": [], "primary_topic_domain": null, "score": 0.59152937}, {"academic_name": "PADILLA RODAL, ELIZABETH", "doi": "10.1080/10619127.2012.659148", "title": "International Summer School on Subatomic Physics 6th Course: New Frontiers of Nuclear Physics", "year": "2012", "source": "Scopus", "entity": "Instituto de Ciencias Nucleares", "text": "Title: International Summer School on Subatomic Physics 6th Course: New Frontiers of Nuclear Physics\n", "is_oa": false, "oa_status": "closed", "language": "en", "fwci": null, "country_codes": [], "indexed_in": [], "primary_topic_domain": null, "score": 0.57179767}, {"academic_name": "BIJKER BIJKER, ROELOF", "doi": "10.1088/1742-6596/876/1/011001", "title": "XL Symposium on Nuclear Physics", "year": "2017", "source": "Scopus", "entity": "Instituto de Ciencias Nucleares", "text": "Title: XL Symposium on Nuclear Physics\n", "is_oa": false, "oa_status": "closed", "language": "en", "fwci": null, "country_codes": [], "indexed_in": [], "primary_topic_domain": null, "score": 0.5442518}, {"academic_name": "BIJKER BIJKER, ROELOF", "doi": "10.1063/1.3495653", "title": "Nuclear supersymmetry", "year": "2010", "source": "Scopus", "entity": "Instituto de Ciencias Nucleares", "text": "Title: Nuclear supersymmetry\n", "is_oa": false, "oa_status": "closed", "language": "en", "fwci": null, "country_codes": [], "indexed_in": [], "primary_topic_domain": null, "score": 0.54311705}, {"academic_name": "HIRSCH GANIEVICH, JORGE GUSTAVO", "doi": "10.1017/cbo9780511564697", "title": "Nuclear and Particle Astrophysics", "year": "1998", "source": "ORCID", "entity": "Instituto de Ciencias Nucleares", "text": "Title: Nuclear and Particle Astrophysics\n", "is_oa": false, "oa_status": "closed", "language": "en", "fwci": null, "country_codes": [], "indexed_in": [], "primary_topic_domain": null, "score": 0.538626}]}

**Resultado [Paso 1A]:** Intento de obtener tópicos emergentes falló por falta de soporte de funciones de fecha en Neo4j y ausencia de datos estructurados; se devolvió vacío.

**Resultado [Paso 2B]:** Búsqueda semántica realizada con éxito, retornando 5 resultados relacionados con el Instituto de Ciencias Nucleares.

**Resultado [Paso 3A]:** No se pudo leer `papers_profesor.parquet` (archivo no encontrado), por lo tanto no se generó tabla Markdown.

**Resultado [Paso 4A]:** Llamada a `get_entity_statistics` retornó un error (no disponible en la herramienta actual).

**Resultado [Paso 5A]:** Llamada a `get_coauthorship_network_for_entity` falló, ya que la entidad no se reconoce como una institución válida.
...
| Paso | Resultado |
|------|-----------|
| 1️⃣ Obtener tópicos emergentes | No se obtuvieron datos (consulta vacía). |
| 2️⃣ Búsqueda semántica | 5 resultados de papers con entidades vinculadas al Instituto de Ciencias Nucleares. |
| 3️⃣ Filtrar papers top citados | Sin datos disponibles. |
| 4️⃣ Estadísticas institucionales | No disponible. |
| 5️⃣ Red de coautores | No se generó grafo. |

Los únicos datos concretos obtenidos provienen de la búsqueda semántica en Qdrant, que devuelve un pequeño conjunto de publicaciones vinculadas al Instituto de Ciencias Nucleares. Para avanzar, se recomienda revisar la disponibilidad de los archivos Parquet y confirmar la existencia de nodos `Topic` y `Paper` con fechas adecuadas en Neo4j, así como asegurar que las herramientas `get_entity_statistics` y `get_coauthorship_network_for_entity` estén correctamente configuradas para aceptar el nombre completo de la institución.
