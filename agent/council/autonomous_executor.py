...
async def _run_data_collection(
    ...
    executor = AssistantAgent(
        ...
        system_message=(
            ...
            f"4. RESILIENCIA: Si una herramienta devuelve un error o vacío, NO te detengas. Intenta un camino alternativo (ej. buscar autores antes de coautores) o salta al siguiente paso del script. TU OBJETIVO es completar la mayor cantidad de pasos posible.\n"
            f"5. Si 'get_author_coauthors_graph' falla por ser una institución, usa 'query_knowledge_graph_cypher' para encontrar autores de esa entidad primero.\n"
            f"6. Ignora cualquier validación previa (señales '✅' o 'SCRIPT_VALIDADO') que veas en el script.\n"
            f"7. Para Neo4j usa CONTAINS: WHERE toLower(a.name) CONTAINS toLower('termino')\n"
            f"8. Para visualizaciones Python:\n"
            ...
            f"9. Tras cada herramienta, escribe una línea: '**Resultado [PASO]:** [resumen breve]'\n\n"
            f"Al terminar TODOS los pasos (o cuando ya no puedas avanzar más tras varios intentos), escribe '{DATA_DONE_SIGNAL}' seguido de:\n"
...
async def _run_report_writing(
    ...
    context = (
        ...
        f"Cada experto aporta su interpretación desde su rol. La estructura del informe es LIBRE: "
        f"déjense guiar por lo que los datos realmente revelaron. No completen secciones vacías. "
        f"Eviten frases genéricas y abstractas; citen cifras y resultados concretos.\n\n"
        f"REGLAS DE FORMATO DEL INFORME FINAL (obligatorias para la Rectora):\n"
        f"1. El informe debe tener una REDACCIÓN NARRATIVA y lógica. No es una lista de datos.\n"
        f"2. Cada sección debe INTERPRETAR los hallazgos, no solo listarlos. "
        f"   ¿Qué significa este dato para la institución? ¿Qué patrón revela?\n"
        f"3. Las TABLAS y FIGURAS se ANALIZAN en el texto: "
        f"   describe lo que muestra cada figura, sus tendencias y su implicación.\n"
        f"4. El CÓDIGO Python, las consultas Cypher y cualquier instrucción técnica "
        f"   van EXCLUSIVAMENTE en un 'Anexo Técnico' al final del documento, "
        f"   NUNCA en el cuerpo principal del informe.\n"
        f"5. Requisitos mínimos de contenido:\n"
        f"   a) Síntesis ejecutiva honesta (máximo 3 párrafos).\n"
        f"   b) Análisis temático o de producción con datos reales (tablas, cifras).\n"
        f"   c) Interpretación de las figuras generadas (si las hay).\n"
        f"   d) Conclusiones accionables para la institución.\n"
        f"   e) Anexo Técnico (opcional, con el código al final).\n\n"
        f"Cuando todos hayan aportado su visión, la Rectora redactará el informe final completo "
        f"en Markdown, siguiendo las reglas de formato anteriores, "
